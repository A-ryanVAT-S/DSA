...
// isalnum(char c) // Checks if character `c` is alphanumeric (digit or letter)//bool
// isspace(char c) // Checks if character `c` is a whitespace (space, tab, newline, etc.)//bool
// isupper(char c) // Checks if character `c` is an uppercase letter (A-Z)//bool
// islower(char c) // Checks if character `c` is a lowercase letter (a-z)//bool
// isxdigit(char c) // Checks if character `c` is a hexadecimal digit (0-9, a-f, A-F)//bool

//     string id=pass.substr(i+1);(for i+1 length to end)
//     string user = pass.substr(0, i + 1);(for i length substr)(i is int not iterator)
